...
def test_reconnects_during_transaction(toxiproxy_db_session, toxiproxy):
    if not toxiproxy:
...
def test_raises_without_using_transaction_retry(
    toxiproxy_db_session, toxiproxy
):
    if not toxiproxy:
        pytest.skip('Toxiproxy not installed')
...
def test_raises_error_if_cannot_reconnect(toxiproxy_db_session, toxiproxy):
    if not toxiproxy:
        pytest.skip('Toxiproxy not installed')
...
class TestTransactionRetryWithDependencyProviders:
    ...
    def test_retries(self, toxiproxy, container):
        if not toxiproxy:
            pytest.skip('Toxiproxy not installed')
    ...
    def test_with_retry_inside(self, toxiproxy, container):
        if not toxiproxy:
            pytest.skip('Toxiproxy not installed')
    ...
    def test_raises_without_retry(self, toxiproxy, container):
        if not toxiproxy:
            pytest.skip('Toxiproxy not installed')
